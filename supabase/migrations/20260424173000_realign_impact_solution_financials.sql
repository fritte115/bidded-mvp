...
set
    ...
    financial_assumptions = coalesce(financial_assumptions, '{}'::jsonb) || jsonb_build_object(
        'revenue_band_sek', jsonb_build_object('min', 21800000, 'max', 24901000),
    ...
    profile_details = coalesce(profile_details, '{}'::jsonb) || jsonb_build_object(
        ...
        'description',
            'Impact Solution Scandinavia AB is an Uppsala-based company focused on IT-based retail solutions and rental vending concepts for workplaces, gyms, golf courses and events. Public annual-account data for 2024 shows 24.901 MSEK revenue, 7 employees, 580 KSEK EBITDA, -104 KSEK result after financial items, 11.187 MSEK total assets and 1.511 MSEK equity.',
        ...
        'financials', jsonb_build_array(
            jsonb_build_object('year', 2020, 'revenue_msek', 12.9,   'ebit_margin_pct', 13.0, 'headcount', 1),
            jsonb_build_object('year', 2021, 'revenue_msek', 17.1,   'ebit_margin_pct', 13.0, 'headcount', 3),
            jsonb_build_object('year', 2022, 'revenue_msek', 19.4,   'ebit_margin_pct', -4.0, 'headcount', 5),
            jsonb_build_object('year', 2023, 'revenue_msek', 21.8,   'ebit_margin_pct', -2.0, 'headcount', 6),
