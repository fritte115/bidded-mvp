...
- **Bidded Prepare/Pending Run Contract**: `prepare_run.py` validates uploaded tender-document sets, runs/reuses ingestion, builds tender/company evidence, records structured preparation audit warnings/errors in run metadata, and then creates one pending `agent_runs` row through `pending_run.py`; manifest workflows register local gitignored seven-PDF sets first and keep procurement roles in document metadata.
...
---
## 2026-04-19 07:14 CEST - US-059
- **Implemented**: Added a JSON manifest workflow and CLI that registers seven local procurement PDFs with source labels/roles before preparing one pending run.
...
---
## 2026-04-19 10:11 CEST - US-069/US-070/US-071/US-072 integration merge
- **Implemented**: Created `codex/merge-ui-with-backend-20260419` from `ralph/bidded-swarm-core`, merged latest `origin/merge-ui-with-backend`, imported the frontend and UI artefacts, wired CLI/API worker runs through the handler factory, and changed runtime selection so `auto` uses Anthropic handlers when `ANTHROPIC_API_KEY` exists while `evidence_locked` remains explicit deterministic mode.
- **Files**: frontend/, src/bidded/api_server.py, src/bidded/cli/__init__.py, src/bidded/config/settings.py, src/bidded/llm/, src/bidded/orchestration/worker.py, src/bidded/orchestration/graph.py, src/bidded/orchestration/judge.py, src/bidded/documents/pdf_ingestion.py, src/bidded/retrieval/__init__.py, tests/test_llm_factory.py, tests/test_settings.py, tests/test_api_server.py, tests/test_cli.py, README.md, .env.example, ralph/prd.json, ralph/state.json, ralph/progress.md
- **Key learnings**: Keep the real Anthropic swarm as the normal demo/runtime path whenever credentials are present, but preserve deterministic handlers as an explicit backend for tests, golden fixtures, and offline rehearsal. The integration branch advances US-069 through US-072, but those stories stay `passes: false` until their remaining dedicated contract-doc, Makefile target, and shared-fixture criteria are completed.
---
