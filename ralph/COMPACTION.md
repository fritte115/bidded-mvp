...
> This file is read ONLY when `ralph/state.json` has `compactionNeeded: true`.
> This session's ONLY job is compacting `ralph/progress.md`. Do NOT work on any story.

## Steps

1. Read `ralph/progress.md` in chunks (use offset/limit, ~300 lines per chunk)
...
4. Write the compacted `ralph/progress.md`
5. Update `ralph/state.json`: set `compactionNeeded: false`, update `progressTokens` with new estimate
