...
- **Bidded Retrieval Contract**: `src/bidded/embeddings.py` owns the 1536-dimension provider/model contract, live/mock adapters, and metadata shape; `src/bidded/documents/chunk_embeddings.py` persists chunk vectors, while `src/bidded/retrieval` ranks hybrid keyword/glossary/embedding candidates with deterministic weights and `match_document_chunks` pgvector rows when live search is available.
