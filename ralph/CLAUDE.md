...
- **Bidded Retrieval Contract**: `src/bidded/embeddings.py` owns the 1536-dimension provider/model contract, live/mock adapters, and metadata shape; `src/bidded/documents/chunk_embeddings.py` persists chunk vectors, while `src/bidded/retrieval` uses `match_document_chunks` for live pgvector search and keyword fallback otherwise.
