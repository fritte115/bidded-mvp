...
- **Bidded Retrieval Contract**: `src/bidded/embeddings.py` owns the 1536-dimension provider/model contract, live/mock adapters, and metadata shape; `src/bidded/documents/chunk_embeddings.py` persists chunk vectors with keyword fallback, while `src/bidded/retrieval` ranks chunks.
