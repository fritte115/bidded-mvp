...
| Tender evidence board | `bidded.evidence` kan föreslå, klassificera, validera, deduplicera, upserta och slå upp `tender_document` evidence rows från retrieved chunks med stabila citation keys och deterministisk nullable `requirement_type`. |
...
| Frontend | Ingen frontend i repot. Lovable är fortsatt tänkt som tunn demo-UI ovanpå Supabase, men `US-027` och framåt prioriterar regulatory glossary, hybrid retrieval och kvalitetshöjande audits innan en ny handoff-story. |
