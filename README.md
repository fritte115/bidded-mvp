...
| Tender evidence board | `bidded.evidence` kan segmentera tenderchunks i klausuler, föreslå, klassificera, validera, deduplicera, upserta och slå upp `tender_document` evidence rows från retrieved chunks med stabila citation keys, deterministisk nullable `requirement_type`, regulatory-glossary metadata, clause-section metadata och contract-clause tag metadata. |
