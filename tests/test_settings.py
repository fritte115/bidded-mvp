...
def test_bidded_anthropic_model_remaps_known_invalid_id() -> None:
    ...
    assert s.bidded_anthropic_model == "claude-3-5-sonnet-20241022"


def test_bidded_anthropic_model_from_env_var_name(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BIDDED_ANTHROPIC_MODEL", "claude-3-5-sonnet-20241022")
    s = BiddedSettings()
    assert s.bidded_anthropic_model == "claude-3-5-sonnet-20241022"
